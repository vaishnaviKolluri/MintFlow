...
actor MockAuthService: AuthServiceProtocol {
    ...
    func updateUser(userId: UUID, fullName: String, email: String) async throws -> User {
        ...
        // Find the user by ID
        guard let entry = userStore.first(where: { $0.value.user.id == userId }) else {
            throw AuthError.invalidCredentials
        }
        
        let oldEmail = entry.key
        let passwordHash = entry.value.passwordHash
        
        // Check if new email is already taken (by another user)
        if newEmail != oldEmail, userStore[newEmail] != nil {
            throw AuthError.userAlreadyExists
        }
        ...
        let updatedUser = User(
            ...
            createdAt: entry.value.user.createdAt
        )
        
        // Remove old entry and add new one
        userStore.removeValue(forKey: oldEmail)
        userStore[newEmail] = StoredCredential(user: updatedUser, passwordHash: passwordHash)
